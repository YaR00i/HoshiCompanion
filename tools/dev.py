...
def run_step(engine: str, log_dir: Path, label: str, args: list[str], timeout: int = 180) -> dict:
    ...
    if label in ['runtime', 'posture', 'shelf', 'shelf_windows', 'external_windows', 'edge_life', 'edge_views', 'cozy_windows', 'place']:
        marker = {'runtime': 'HOSHI_TEST_RESULT', 'posture': 'HOSHI_POSTURE_RESULT', 'shelf': 'HOSHI_SHELF_RESULT', 'shelf_windows': 'HOSHI_SHELF_RESULT', 'external_windows': 'HOSHI_EXTERNAL_RESULT', 'edge_life': 'HOSHI_EDGE_LIFE_RESULT', 'edge_views': 'HOSHI_EDGE_CAPTURE_RESULT', 'cozy_windows': 'HOSHI_COZY_RESULT', 'place': 'HOSHI_PLACE_RESULT'}[label]
...
def main() -> int:
    ...
    parser.add_argument('mode', choices=['check', 'import', 'test', 'smoke', 'preview', 'desktop', 'editor', 'walk', 'capture', 'posture', 'poses', 'shelf', 'shelf_windows', 'shelf_demo', 'external_windows', 'edge_life', 'edge_views', 'cozy_windows', 'place'])
    ...
    stages = []
    if opts.mode in ['check', 'import', 'test', 'preview', 'desktop', 'editor', 'walk', 'capture', 'posture', 'poses', 'shelf', 'shelf_windows', 'shelf_demo', 'external_windows', 'edge_life', 'edge_views', 'cozy_windows', 'place']:
